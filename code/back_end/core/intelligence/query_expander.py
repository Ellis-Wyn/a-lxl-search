...
将用户输入的靶点名称扩展为完整的PubMed查询：
- EGFR → EGFR OR ERBB1 OR HER1 OR Epidermal Growth Factor Receptor
- 智能处理同义词、基因名称、全名
- 提升搜索召回率和准确率

使用示例：
    from core.intelligence import QueryExpander

    expander = QueryExpander()
    expanded = expander.expand("EGFR")
    # Output: ("EGFR" OR "ERBB1" OR "HER1" OR "Epidermal Growth Factor Receptor")
...
class QueryExpander:
    ...
    def get_target_info(self, target_name: str) -> Optional[Dict[str, str]]:
        """
        获取靶点信息
        ...
        Example:
            >>> expander = QueryExpander()
            >>> info = expander.get_target_info("EGFR")
        ...
        # 精确匹配
        if target_name_upper in self.TARGET_GENE_MAPPING:
            return self.TARGET_GENE_MAPPING[target_name_upper]

        # 模糊匹配
        for key, value in self.TARGET_GENE_MAPPING.items():
    ...
    def expand(
        ...
        Args:
            ...
            output_format: 输出格式 ("pubmed" 或 "simple")
        ...
        Example:
            >>> expander = QueryExpander()
            >>> query = expander.expand("EGFR")
            >>> print(query)
            ("EGFR" OR "ERBB1" OR "HER1" OR "Epidermal Growth Factor Receptor")

            >>> # Simple format (for unified search)
            >>> query = expander.expand("EGFR", output_format="simple")
            >>> print(query)
            ("EGFR" OR "ERBB1" OR "HER1" OR "Epidermal Growth Factor Receptor")
        """
        ...
        if output_format == "pubmed":
            # PubMed格式：带字段限定
            term_queries = [f'"{term}"' for term in search_terms]
            ...
            # Simple格式：简单的OR连接
            term_queries = [f'"{term}"' for term in search_terms]
            query_str = " OR ".join(term_queries)

            # 添加全名
            if include_full_name and info.get("full_name"):
                query_str += f' OR "{info["full_name"]}"'

            return f"({query_str})"

    def expand_to_list(self, target_name: str) -> List[str]:
        """
        扩展查询为列表形式（用于数据库搜索）
        ...
        Example:
            >>> expander = QueryExpander()
            >>> terms = expander.expand_to_list("EGFR")
            >>> print(terms)
        ...
        # 构建搜索词列表
        search_terms = [info["standard_name"]]
        ...
        if info.get("full_name"):
            search_terms.append(info["full_name"])
    ...
    def get_all_target_names(self) -> List[str]:
        """获取所有已知的靶点名称"""
        return list(self.TARGET_GENE_MAPPING.keys())
    ...
    def search_by_keyword(self, keyword: str) -> List[str]:
        ...
        Example:
            >>> expander = QueryExpander()
            >>> results = expander.search_by_keyword("EGF")
            >>> print(results)
            ['EGFR']
        """
        ...
        for target_name, info in self.TARGET_GENE_MAPPING.items():
            # 检查标准名称
