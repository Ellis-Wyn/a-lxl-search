...
使用示例：
    from core.intelligence import PhaseNormalizer

    normalizer = PhaseNormalizer()
    phase = normalizer.normalize("III期")
    # -> "Phase 3"

    # 比较阶段
    if normalizer.is_later_phase("Phase 3", "Phase 2"):
...
class PhaseNormalizer:
    ...
    # Phase映射表
    PHASE_MAPPING: Dict[str, StandardPhase] = {
        # ===== Phase 1 =====
    ...
    def __init__(self):
        ...
        for key, value in self.PHASE_MAPPING.items():
    ...
    def normalize(self, phase_str: Optional[str]) -> str:
        ...
        Examples:
            >>> normalizer = PhaseNormalizer()
            >>> normalizer.normalize("Phase II")
            'Phase 2'
            >>> normalizer.normalize("二期临床")
            'Phase 2'
        """
        if not phase_str:
        ...
        # 精确匹配
        if phase_str in self.PHASE_MAPPING:
            return self.PHASE_MAPPING[phase_str].value
        ...
        # 关键词匹配
        for key, standard_phase in self.PHASE_MAPPING.items():
    ...
    def get_order(self, phase_str: Optional[str]) -> int:
        ...
        return PHASE_ORDER[StandardPhase.UNKNOWN]
    ...
    def is_later_phase(
        ...
        Examples:
            >>> normalizer = PhaseNormalizer()
            >>> normalizer.is_later_phase("Phase 3", "Phase 2")
            True
            >>> normalizer.is_later_phase("Phase 2", "Phase 3")
            False
        """
...
def get_phase_normalizer() -> PhaseNormalizer:
    """获取PhaseNormalizer单例"""
    global _normalizer_instance
    if _normalizer_instance is None:
        _normalizer_instance = PhaseNormalizer()
